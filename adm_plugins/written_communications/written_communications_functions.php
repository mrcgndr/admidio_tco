...
$getRecipientGender         = admFuncVariableIsValid($_POST, 'plg_wc_recipient_gender', 'string', array('defaultValue' => ''));
...
$getSenderGender            = admFuncVariableIsValid($_POST, 'plg_wc_sender_gender', 'string', array('defaultValue' => ''));
$getSenderPhone         	= admFuncVariableIsValid($_POST, 'plg_wc_sender_phone', 'string', array('defaultValue' => ''));
$getSenderMobile            = admFuncVariableIsValid($_POST, 'plg_wc_sender_mobile', 'string', array('defaultValue' => ''));
...
if(isset($_POST['sender_user']))
{
    ...
    $arrSender = array( 'Sender_Organization'   => $gCurrentOrganization->getValue('org_longname'),
                        ...
                        'Sender_Gender'         => $gCurrentUser->getValue('GENDER'),
                        ...
                        'Sender_Phone'			=> $gCurrentUser->getValue('PHONE'),
                        'Sender_Mobile'			=> $gCurrentUser->getValue('MOBILE'));
}
else
{
    ...
    if(count($arrMembers) > 0)
    {
        foreach($arrMembers as $memberData)
        {
            $arrRecipient[] = array('Recipient_Organization'    => '',
                                    ...
                                    'Recipient_Gender'          => $memberData['gender'],
...
else
{
    ...
    $arrRecipient[] = array('Recipient_Organization'   => $getRecipientOrganization,
                            ...
                            'Recipient_Gender'         => $getRecipientGender,
...
foreach($arrRecipient as $Recipient)
{
    ...
    if($Recipient['Recipient_Gender'] == 1)
    {
    	$document->setValue('Recipient_Gender', 'Herr');
    }
    elseif($Recipient['Recipient_Gender'] == 2)
    {
    	$document->setValue('Recipient_Gender', 'Frau');
    }
    else
    {
    	$document->setValue('Recipient_Gender', $Recipient['Recipient_Gender']);	
    }
    ...
    if($arrSender['Sender_Gender'] == 1)
    {
    	$document->setValue('Sender_Gender', 'Herr');
    }
    elseif($arrSender['Sender_Gender'] == 2)
    {
    	$document->setValue('Sender_Gender', 'Frau');
    }
    else
    {
    	$document->setValue('Sender_Gender', $arrSender['Sender_Gender']);	
    }
    ...
    $document->setValue('Sender_Phone', $arrSender['Sender_Phone']);
    $document->setValue('Sender_Mobile', $arrSender['Sender_Mobile']);
